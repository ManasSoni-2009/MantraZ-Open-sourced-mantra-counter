...
self.onmessage = (event) => {
  ...
  const { sessionId, transcript, triggers, countOn, sensitivity } = payload;
  const session = getSession(sessionId);
  const normalizedTokens = normalizeTranscript(transcript);
  const triggerSet = buildTriggerSet(triggers || [], sensitivity);
  const countSet = buildTriggerSet(countOn || [], sensitivity);
  ...
  const countTokens = freshTokens.filter((token) => tokenMatches(token, countSet, sensitivity));
