document.addEventListener('DOMContentLoaded', () => {
  ...
  const DEFAULT_MANTRAS = [
    { id: '1', name: 'Gayatri Mantra', text: 'Om Bhur Bhuva Swaha...', deity: 'Savitr', category: 'vedic', lang: 'Sanskrit', target: 108, triggers: ['om', 'bhur', 'swaha', 'svaha'], countOn: ['swaha', 'svaha'] },
    { id: '2', name: 'Shiva Panchakshara', text: 'Om Namah Shivaya', deity: 'Shiva', category: 'shaiva', lang: 'Sanskrit', target: 108, triggers: ['om', 'namah', 'shiva', 'shivaya'], countOn: ['shivaya'] },
    { id: '3', name: 'Maha Mrityunjaya', text: 'Om Tryambakam Yajamahe...', deity: 'Shiva', category: 'shaiva', lang: 'Sanskrit', target: 108, triggers: ['om', 'tryambakam', 'yajamahe', 'swaha'], countOn: ['swaha', 'svaha'] },
    { id: '4', name: 'Hare Krishna', text: 'Hare Krishna Hare Krishna...', deity: 'Krishna', category: 'vaishnava', lang: 'Sanskrit', target: 108, triggers: ['hare', 'krishna', 'rama'], countOn: ['rama'] },
  ];

  let settings = { ...DEFAULT_SETTINGS, ...(JSON.parse(localStorage.getItem('mantraz_settings')) || {}) };
  let mantras = (JSON.parse(localStorage.getItem('mantraz_mantras')) || DEFAULT_MANTRAS).map(normalizeMantra);
  ...
  if (worker) {
    worker.onmessage = ({ data }) => {
      ...
      const { increments, normalizedTranscript, matchedTokens, countTokens } = data.payload;
      ...
      if (increments > 0) {
        ...
        const countLabel = (countTokens && countTokens.length ? countTokens : matchedTokens).join(', ');
        showToast(`+${increments} on ${countLabel}`);
      }
    };
  ...
  function normalizeMantra(mantra) {
    const triggers = Array.isArray(mantra.triggers) ? mantra.triggers.map((trigger) => String(trigger).trim().toLowerCase()).filter(Boolean) : [];
    const countOn = Array.isArray(mantra.countOn) && mantra.countOn.length
      ? mantra.countOn.map((token) => String(token).trim().toLowerCase()).filter(Boolean)
      : (triggers.length ? [triggers[triggers.length - 1]] : []);
  ...
  function loadMantra(mantra) {
    ...
    activeMantraTriggers.textContent = `${mantra.triggers.join(', ')} • count on ${mantra.countOn.join('/')}`;
    resetSession();
  ...
  formEditTriggers.addEventListener('submit', (event) => {
    ...
    currentMantra.countOn = [words[words.length - 1]];
    const mantraIndex = mantras.findIndex((mantra) => mantra.id === currentMantra.id);
    if (mantraIndex !== -1) mantras[mantraIndex] = normalizeMantra(currentMantra);
    activeMantraTriggers.textContent = `${words.join(', ')} • count on ${currentMantra.countOn.join('/')}`;
    persistMantras();
  ...
  document.getElementById('mantra-form').addEventListener('submit', (event) => {
    ...
    const mantra = {
      ...
      target,
      triggers,
      countOn: [triggers[triggers.length - 1]],
      category: 'custom'
    };
    mantras.unshift(normalizeMantra(mantra));
    persistMantras();
    renderMantraLibrary();
    renderHistory();
    closeModal(modalAdd);
